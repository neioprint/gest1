...
//echo $occurence."<br";
if ($occurence > 0) {
  $_SESSION['erreur'] = "Deux Clients ne peuvent pas avoir la même Facture proforma!";
...
if (@$_SESSION['valider'] === "oui") {
  ...
  if ($_SESSION['valider'] != "deja" && $_SESSION['valider'] === "oui") {
    ...
    //die();
    $client = $nomclient;
    $fichier = "prof" . strtolower($client) . $nbr . ".pdf";
    //$datebl="2022/05/05";
    $montant = $total1;
...
<body>
  ...
  <div class="containerbl">
    <div class="stylebl">
      ...
      <form name="form" method="get" action="">
        <p class="gras"><span class="gras">Facture Proforma N°</span><?php if ($nbr > 0) echo @$nbr ?> du</p>
  ...
  <p>Arrétée la présente Facture Proforma à la somme de <span class="gras"> <?= " " . $chaine ?> </span></p>
  <a id="bouton" href="./indexcommande.php?page=1&recherche=&niveau=ins" class="btn btn-danger btn-block">Retour ou Annuler</a>
  <a id="bouton" href="validerproforma.php?idclient=<?= $trieclientid ?>&datebl=<?= $datebl ?>&modepaiement=<?= $modepaiement ?>&nbr=<?= $nbr ?>" class="btn btn-primary btn-block">Valider La Facture Proforma</a>
