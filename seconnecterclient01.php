...
//echo $login;
    ...
    if (empty($_SESSION['pass'])) {
        ...
        foreach ($resultclient as $client) {
            ...
            if ($client['email'] == $login && empty($_SESSION['trouve'])) {
                ...
                if(!isset($_COOKIE['aleatoire']) && !isset($_COOKIE['login'])&& $pasdecookie==1) {
                    ...
                    setcookie('aleatoire', $aleatoire, time() + (150), "/"); // 86400 = 1 day
                    setcookie('login', $login, time() + (150), "/"); // 86400 = 1 day
                } 
        $msg = "Mr $login votre code de securité à usage unique est: $aleatoire";
        ...
        mail($login, "CODE DE SECURITE A USAGE UNIQUE", $msg, $headers);
                // die();
