...
$infos = $datepointage . '_' . $heurepointage . '_' . $salarie;
//echo $infos;
...
"></script>
<link href="
https://cdn.jsdelivr.net/npm/sweetalert2@11.7.22/dist/sweetalert2.min.css" rel="stylesheet">
...
<body>
  ...
  <div class="container">
    ...
    <?php
    if ($heurepointage > date("07:00") && $heurepointage < date("23:59"))
    // if (1==1)
    { ?>

      <?php if ($pointage == false) { ?>
        <button class="btn btn-primary btn-lg btn-success" onclick="envoipointage(1)">Pointage Entree</button>
      <?php   } ?>

      <?php if (@$pointage['sortie2'] == "" && $pointage != false) { ?>
        <button class="btn btn-primary btn-lg btn-danger" onclick="envoipointage(4)">Pointage Sortie</button>
        <!-- <button class="btn btn-primary btn-lg btn-success" onclick="envoipointage(2)">Sortie1</button> -->

      <?php   } ?>

      <?php if (@$pointage['sortie1'] != "" && @$pointage['sortie2'] != "") { ?>
        <button class="btn btn-primary btn-lg btn-danger" onclick="">Pointage deja fait</button>
        <!-- <button class="btn btn-primary btn-lg btn-success" onclick="envoipointage(2)">Sortie1</button> -->
...
< ?php } ?> -->
    ...
    <button class="btn btn-primary btn-lg btn-danger" onclick="history.back()">Retour</button>
  ...
  <script>
    function envoipointage(entree) {
      ...
      if (entree == 1) {
        ...
        Swal.fire({
          title: `Confirmation pointage ENTREE? ${hours}`,
          text: "Etes vous sûr?",
          ...
          // Toast.fire({
          //   icon: 'success',
          //   title: 'Camera en cours...'
           
          // }).then((result) => {

           
             document.location.href = "camera2.php?pointage=1&infos=<?= @$infos ?>";
          // })
      ...
      if (entree == 2) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            // }) // fin is result;
            document.location.href = "camera2.php?pointage=2&infos=<?= @$infos ?>";
          } // fin is confirmedresult
      ...
      if (entree == 3) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            // //  document.location.href="confirmationpointage.php?pointage=3&infos=< ?=@$info?>";
            // // // document.location.href='camera2.php?pointage=3';
            //  }) // fin is result;
            document.location.href = "camera2.php?pointage=3&infos=<?= @$infos ?>";
          } // fin is confirmedresult
      ...
      if (entree == 4) {
        Swal.fire({
          title: `Confirmation pointage de Fin SORTIE?${hours}`,
          text: "Etes vous sûr?",
          ...
          if (result.isConfirmed) {
            ...
            // }) // fin is result;
            //document.location.href='confirmationpointage.php?pointage=4';
            document.location.href = "camera2.php?pointage=4&infos=<?= @$infos ?>";
